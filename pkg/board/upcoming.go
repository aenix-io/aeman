...
// task comes due and has no turn of its own yet.
//
// A week whose turn is already filed is left out: that turn is a card, and
// the board draws cards. What is left is what is coming — the turns nobody
// can act on yet but everybody has to plan around.
//
...
func UpcomingTurns(b Board, task Card, from string, weeks int) []string {
	...
	for i := 0; i < weeks; i++ {
		...
		if !filed[week] && DueInWeek(task.Recurrence, task.StartDate, week) {
			out = append(out, week)
		}
	}
	return out
