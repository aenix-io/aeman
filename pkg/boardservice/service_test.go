...
func (f *fakeBackend) SetDoneAt(_ context.Context, _ board.Board, card board.Card, day string) error {
	f.mu.Lock()
	defer f.mu.Unlock()
...
func TestRemoveGridDeletesFromCurrentSprint(t *testing.T) {
	f := newFake([]board.Card{{ItemID: "c1", Team: "alpha", StartDate: "2026-01-10",
		...
		map[string]board.SprintState{"alpha": {Current: "2026-01-10", Previous: "2026-01-03"}})
	if err := f2svc(f).Remove(ctx, "acme", "c1", board.ViewAll, RemoveAuto); err != nil {
		t.Fatal(err)
	}
	if c := f.get("c1"); c != nil {
...
func TestGridRemoveOnACardTakenOutOfItsWeek(t *testing.T) {
	...
	for _, id := range []string{"fresh", "worked"} {
		if err := svc.Remove(ctx, "acme", id, board.ViewAll, RemoveAuto); err != nil {
			t.Fatal(err)
		}
		c := f.get(id)
		if c == nil {
...
func TestTheSmartCrossDeletesAWorkedCard(t *testing.T) {
	...
	svc := New(fake)
	if err := svc.Remove(t.Context(), "o", "c1", board.ViewAll, RemoveAuto); err != nil {
		t.Fatal(err)
	}
