...
func TestTheRemoveNeverDeletesAProjectCard(t *testing.T) {
	...
	if err := New(fake).Remove(t.Context(), "o", "c1", board.ViewAll, RemoveAuto); err != nil {
		t.Fatal(err)
	}
	b, _ := fake.LoadBoard(t.Context(), "o")
	c, ok := findCard(b, "c1")
...
func TestSubtasksLeaveTheWorkingAreaWithTheirParent(t *testing.T) {
	...
	if err := New(fake).Remove(t.Context(), "o", "p", board.ViewAll, RemoveAuto); err != nil {
		t.Fatal(err)
	}
	b, _ := fake.LoadBoard(t.Context(), "o")
	sub, ok := findCard(b, "s")
...
func TestTheRemoveOnASubtaskDeletesItRatherThanDemotingIt(t *testing.T) {
	...
	if err := New(fake).Remove(t.Context(), "o", "s", board.ViewAll, RemoveAuto); err != nil {
		t.Fatal(err)
	}
	b, _ := fake.LoadBoard(t.Context(), "o")
