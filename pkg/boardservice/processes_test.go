...
func TestTheCrossNeverDeletesASlot(t *testing.T) {
	...
	if err := New(fake).Remove(context.Background(), "acme", "slot", board.ViewAll, RemoveAuto); err != nil {
		t.Fatal(err)
	}
	got := fake.get("slot")
	if got == nil {
...
func TestTheCrossNeverDeletesAProcessTurn(t *testing.T) {
	...
	svc := New(fake)
	if err := svc.Remove(context.Background(), "acme", "turn", board.ViewAll, RemoveAuto); err != nil {
		t.Fatal(err)
	}
	got := fake.get("turn")
	if got == nil {
