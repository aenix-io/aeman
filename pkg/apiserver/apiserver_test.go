...
// The resource says when a card was done (status.doneAt), which repository
// it lives in, and the day the older × left it behind on.
func TestCardResourceCarriesDoneAtAndDomain(t *testing.T) {
	b := testBoard()
	c := b.Cards[0]
	c.Progress, c.DoneAt, c.Domain, c.LeftAt = 100, "2026-08-28", "closed", "2026-08-27"
	r := CardResource(b, c)
	if r.Status.DoneAt != "2026-08-28" || r.Status.Domain != "closed" || r.Status.LeftAt != "2026-08-27" {
