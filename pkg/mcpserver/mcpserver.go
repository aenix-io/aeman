...
func (h *server) mcpServer() *mcp.Server {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "add_epic", Description: "Declare a new epic — a column of the Project board (weeks as rows, a project's epics as columns) — inside an existing project, which is required: pass project=<name> from get_board metadata.projects (add_project creates one). Column names are unique WITHIN a project, so every project can have its own \"Docs\"; a card names the (project, epic) pair. Cards are filed under a column via create_card/update_card epic=<name>+project=<name>; their week is the row and start/end may span several weeks. Only on an explicit request for a new epic: read the existing ones from get_board metadata.epics first."}, h.addEpic)
