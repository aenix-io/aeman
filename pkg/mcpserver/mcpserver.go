...
func (h *server) mcpServer() *mcp.Server {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "carry_week", Description: "File the process turns a team's week is owed, and reseed its finished recurrent cards (dryRun reports the counts). The server does this by itself as each week arrives; call it only to force the sweep now. Unfinished plan cards are NOT moved: an open card from an earlier week is a debt — it stays in the week it was owed in and shows on the current week's panel with the overdue mark, and the report's carried count is how many such debts there are."}, h.carryWeek)
