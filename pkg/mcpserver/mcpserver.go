...
func (h *server) mcpServer() *mcp.Server {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "remove_from_project", Description: "The Project board's x: remove the card from ONE column ({uid, project, epic}). A mirror simply goes; the home column, with mirrors left, hands its role to the first mirror; the LAST column also drops the card from the weekly plan always, and the card survives only in the working area — only when someone had it and moved it (assignee + progress). An untouched card is DELETED outright — ask the person first when work would go. A SUBTASK is never deleted here: its other home is its parent, so the x only takes the column away."}, h.removeFromProject)
	...
	mcp.AddTool(s, &mcp.Tool{Name: "release_from_plan", Description: "Take a card out of the weekly plan. A card that is somewhere else — in a Project-board column (the EPIC side: a bare project name is on no board), or in the working area by its sprint or its dates — only loses its plan band and week. Otherwise the plan was its last home and the card is DELETED, with its linked review card, however much work is on it and whoever it is assigned to: being on a person or carrying progress is not a place to be. Ask the person before calling this on a card that carries work."}, h.releaseFromPlan)
