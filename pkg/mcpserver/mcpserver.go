...
func (h *server) mcpServer() *mcp.Server {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "set_capacity", Description: "Set how many points a week a person gets through — the number the day boards measure their load against (get_board metadata.members[].capacity, beside load). 0 takes a set number back: the board then derives one from what the person has been closing over the last four complete weeks (capacityDerived=true). Set one when you know better than the record — a half week, a newcomer, somebody covering for two."}, h.setCapacity)
