...
func (h *server) mcpServer() *mcp.Server {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "get_board", Description: "Get the board identity, its team roster (metadata.teams) and the Project board's structure: the project roster (metadata.projects) and the epic columns with the project each belongs to (metadata.epics). \"Project\" here is aeman's planning entity — a group of epic columns — not the GitHub board, which is addressed by owner+board."}, h.getBoard)
	...
	mcp.AddTool(s, &mcp.Tool{Name: "add_epic", Description: "Declare a new epic — a column of the Project board (weeks as rows, a project's epics as columns) — inside an existing project, which is required: pass project=<name> from get_board metadata.projects (add_project creates one). Cards are filed under an epic via create_card/update_card epic=<name>; their week is the row and start/end may span several weeks. Only on an explicit request for a new epic: read the existing ones from get_board metadata.epics first."}, h.addEpic)
	mcp.AddTool(s, &mcp.Tool{Name: "delete_epic", Description: "Delete an EMPTY epic column from the Project board. An epic that still has cards is protected (move or clear them first) — orphaning planned work silently is exactly what the Project board exists to prevent."}, h.deleteEpic)
	mcp.AddTool(s, &mcp.Tool{Name: "set_epic_project", Description: "Move an epic column to another project (name=<epic>, project=<project>). An empty project detaches the column from every project, leaving it visible only in the all-projects view — do that only when explicitly asked. The epic's cards ride along: a card's project always follows its epic."}, h.setEpicProject)
