...
func (h *server) mcpServer() *mcp.Server {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "remove_from_project", Description: "The Project board's x: remove the card from ONE column ({uid, project, epic}). A mirror simply goes; the home column, with mirrors left, hands its role to the first mirror; the LAST column also drops the card from the weekly plan always, and the card survives only in the working area — only when someone had it and moved it (assignee + progress). An untouched card is DELETED outright — ask the person first when work would go."}, h.removeFromProject)
