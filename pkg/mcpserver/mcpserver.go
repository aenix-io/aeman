...
func (h *server) mcpServer() *mcp.Server {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "get_board", Description: "Get the board identity, its team roster (metadata.teams) and the Plan board's epic columns (metadata.epics)."}, h.getBoard)
	...
	mcp.AddTool(s, &mcp.Tool{Name: "add_epic", Description: "Declare a new epic — a column of the Plan board (weeks as rows, epics as columns). Cards are filed under an epic via create_card/update_card epic=<name>; their week is the row and start/end may span several weeks. Only on an explicit request for a new epic: read existing ones from get_board metadata.epics first."}, h.addEpic)
	mcp.AddTool(s, &mcp.Tool{Name: "delete_epic", Description: "Delete an EMPTY epic column from the Plan board. An epic that still has cards is protected (move or clear them first) — orphaning planned work silently is exactly what the Plan board exists to prevent."}, h.deleteEpic)
