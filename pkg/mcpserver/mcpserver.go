...
func (h *server) mcpServer() *mcp.Server {
	...
	mcp.AddTool(s, &mcp.Tool{Name: "list_processes", Description: "The Process tab: recurring work the team keeps doing — every process with its templates, and each template's history (done / open / late per iteration), which is how to tell whether a process is actually alive. Scope with project=<name>."}, h.listProcesses)
	mcp.AddTool(s, &mcp.Tool{Name: "add_process", Description: "Declare a process — recurring work inside a project, e.g. \"Publishing\" or \"Collecting payment\". A process groups templates; add them with add_process_template. Only on an explicit request: read the existing ones from list_processes first."}, h.addProcess)
	mcp.AddTool(s, &mcp.Tool{Name: "delete_process", Description: "Delete an EMPTY process. One that still has templates is protected: delete those first, on purpose."}, h.deleteProcess)
	mcp.AddTool(s, &mcp.Tool{Name: "rename_process", Description: "Rename a process (name=<current>, to=<new>); its templates follow."}, h.renameProcess)
	mcp.AddTool(s, &mcp.Tool{Name: "set_process_project", Description: "Move a process to another project (project=\"\" is the no-project bucket). Its templates and their iterations are untouched: a process belongs to a project, the work it spawns belongs to the process."}, h.setProcessProject)
	mcp.AddTool(s, &mcp.Tool{Name: "add_process_template", Description: "Add what a process iterates on: the title and body every iteration is created with, its cycle (week | 2weeks | month | quarter, counted on the calendar from start), the team whose weekly plan the iterations land in, and the standing owner. Every iteration is a fresh copy of THIS — a renamed live card stays renamed, the next one is the template again. carry_week spawns what each week is owed. The cycle is per template because one process (collecting payment) has one template per client, and clients pay on different schedules."}, h.addProcessTemplate)
	mcp.AddTool(s, &mcp.Tool{Name: "update_process_template", Description: "Change what the NEXT iterations will be (only the provided fields apply). The iteration currently running is left exactly as it is."}, h.updateProcessTemplate)
	mcp.AddTool(s, &mcp.Tool{Name: "delete_process_template", Description: "Delete a template. Its past iterations are ordinary cards and stay — they are the record of what was done."}, h.deleteProcessTemplate)
