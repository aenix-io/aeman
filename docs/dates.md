...
- **The past begins where the running sprint does**, not at midnight. A sprint lays itself out on its own day — that is where the lead works it from, and where a card created today lands (its `sprintStart` may be days old) — so a day inside it is live and editable however far back the calendar has moved. The boundary is the **earliest** running sprint among the teams in view, because a day still inside anyone's sprint belongs to them: on the Team board, the teams shown; on Me, the teams of that person's cards. A team whose sprint has stood since July therefore keeps every day since July live — that sprint IS open, and Carry Over is what closes it.
