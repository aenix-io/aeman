...
- **The day a SPRINT began shows that sprint's own work**, whatever has become of it since — including the cards created after the sprint opened, which is most of them: a sprint that opened on Monday and is read on Wednesday would otherwise show almost none of its own work. That is the view a team opens to read the sprint it is in, and the day the day-navigator jumps to. A card deferred past **today** is the one exception, because deferring is the act of taking a card out of the sprint in progress.
...
- **A day says which of its cards were finished on it.** A day board draws finished work on the day it recorded (`doneAt`) and no other — and the record of a day can supply that day where the card's own writer did not: the day's commits name the cards they touched, so a card that is done in the day's tree and was written during the day was finished during the day. It is filled in on the way out of the history, never stored. A card the day did not touch is not the day's, and keeps whatever it says itself.
