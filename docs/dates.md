...
- **The day a SPRINT began also holds the work created inside it**, which is most of a sprint: a card typed on the Tuesday of a sprint that opened on Monday is that sprint's work, and a day that only showed what existed on the Monday would show almost none of it by Wednesday. That is the view a team opens to read the sprint it is in, and the day the day-navigator jumps to. It is not an exemption from the rest of the rule: work planned into a later week still waits for that week, finished work still belongs to the day it was finished on, and a card deferred past **today** leaves that day at once, because deferring is the act of taking a card out of the sprint in progress. So the sprint's day shows the sprint's work that is IN HAND — not a report of what the sprint has done.
