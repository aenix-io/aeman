...
- `POST /api/v1/cards` and `update_card` accept `lane` and `week`. A `lane` on a card whose lane derives is refused (422): the links say what it is.
...
- MCP: `list_cards view=triage` with the same selector. There is no `place_card` tool: a week is set with `update_card week=<Monday>` and taken away with `update_card week=""`. Note that this is `SetWeek`, not the REST `place` action — it writes the week and nothing else, where `place` also empties the working area for a week AHEAD (the card is on no day board until then) and joins the team's sprint for the CURRENT one. An agent scheduling work for a later week should clear the dates itself, or the card stands on today's board and in a future week at once.
