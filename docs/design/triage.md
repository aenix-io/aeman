...
>
> A card with a `week` ahead of the current one is **placed**, not scheduled: it is on no day board (Team, Me, the personal column) until its Monday, whatever its dates say — pulled onto a day from there. A placed card is owed by its week's Friday. A placed card still open when its week has passed is a debt and is not moved (see Overdue). The Triage board shows the weeks side by side; the current week's column is that week's cards plus the current sprint, and a future week is partly projected — Project-board slots by their span, process turns by their cycle.
