...
Who does what, and in which week. It is the fourth board — Me | Team | **Triage** | Project | Process — and it is not a new kind of thing: a card in the backlog is a card whose week has not come yet. Decided 2026-09-02 after the leads' meeting; the rules are to be pinned in [behavior-matrix.md](behavior-matrix.md) rows B1–B10 as they land.

> **v0.20.** This board replaced the weekly plan, which is gone: the panel under the Team grid, its wed/fri bands and the actions that moved cards between them. Where this document says "the Weekly panel", read "this board's column for that week"; the `plan` band no longer exists, and a card's week is `spec.week` on its own. The matrix is the authority on what actually landed.
...
- **Week.** A card on the board is a card with `week` (a Monday), placed in a week ahead. Nothing about the card says "triage"; the Triage board is the weeks laid side by side. When the week arrives the card stands in the current week's column; it is pulled onto a day at the daily sync; if it is still open when its week has passed it is **overdue** and stays in the week it was owed in, shown beside the current week's work (see [processes.md](processes.md), "Overdue, and why nothing moves"). None of that is new. What is new is one rule: **a card whose week lies ahead is on no day board** — not on Team, not on Me — until its week begins. That rule is what makes the backlog a regulator rather than a list: a card placed three weeks out cannot be picked up on Tuesday because it caught someone's eye.
...
- **Capacity.** *(Superseded — see the note below.)* How many cards a team finishes in a week, in `teams/<id>.yaml`:
...
> **Superseded.** The cards-a-week capacity above, its `client`/`internal` shares and the derivation behind them were removed (B7): nothing ever wrote the block — not one team file on the production board carried it — and no client read it back, the bar it was drawn for having gone from the boards. The paragraph's own caveat is why: a number derived from four weeks of `doneAt` is unreliable while `doneAt` is younger than four weeks, and it stayed unreliable without ever saying so. What a week is weighed against now is a team's **points** a week (`capacity: points:`), in the same file, and somebody SETS it — B20, with the arithmetic in the `derive-capacity` skill. The unit changed with it: a card is no longer the measure, a SIZE is (B18), so the "size norm" this section asks of the process is now the board's own arithmetic.
...
- **Dragging.** Between columns sets `week` (a slot's week moves the slot, as on the Project board — same card, same field); between lanes sets `lane`; into the current week sets this Monday and the card is back on the day boards at once; into the triage strip clears `week`. A debt dragged into the current week is a debt no more — it was owed, and it is re-owed.
...
Rows for [behavior-matrix.md](behavior-matrix.md), to be added with the tests that pin them; the tests are written first, one failing case per row, in `pkg/board` (`backlog_test.go`) and `pkg/boardservice` (`triage_test.go`), with the vitest mirror in `triage.test.ts`.

This table is the PROPOSAL as it was written on 2026-09-02, kept as the record of what was asked for. Some of it landed as written, some landed changed, and some (the lanes, the signals) never landed at all — [behavior-matrix.md](behavior-matrix.md) is the authority on what the code does, and where the two disagree the matrix is right. A test named here may no longer exist; the matrix's own third column is the one that is checked.

| # | Rule | Lives in | Test |
| --- | --- | --- | --- |
| B1 | A card with `week` ahead of today's week is on no day board — Team, Me — until that week begins; from its Monday it shows on the day boards by the rules they have today. A card with `week` set is owed by that week's Friday | board.TeamGrid / MeView; boardservice.Place | board `TestACardPlacedInAWeekAheadIsOnNoDayBoardUntilItsWeek` |
| B1a | Placed back in the CURRENT week, a card standing on no day joins the sprint being worked (the team's active sprint, else its current one, else the week itself) — the round trip out to a week ahead and back must not leave the card holding a week and standing on no board. A card that already has a day keeps it; a slot is left alone | boardservice.Place | boardservice `TestPlacingBackInThisWeekPutsTheCardOnTheDayBoardAgain` |
...
| B7 | ~~Capacity is the team's: `capacity.week` if set, else the team's cards with `doneAt` in the last four complete weeks over four~~ — **removed.** Nothing ever wrote the number and no client read it back. A team's week is measured in POINTS now, somebody sets it, and a PERSON has one too (matrix B19, B20) | — | — |
...
>
> A card with a `week` ahead of the current one is **placed**, not scheduled: it is on no day board (Team, Me) until its Monday, whatever its dates say — pulled onto a day from there. A placed card is owed by its week's Friday. A placed card still open when its week has passed is a debt and is not moved (see Overdue). The Triage board shows the weeks side by side; the current week's column is that week's cards plus the current sprint, and a future week is partly projected — Project-board slots by their span, process turns by their cycle.

## Surface

- `GET /api/v1/views/triage/cards?team=<name>[,<name>]&from=<monday>&weeks=<n>` — every placed card of the teams with `week` in `[from, from + weeks)`, the current week's sprint cards, the triage strip (`status.triage: true`), the ghosts (`kind: Ghost`, no uid), each card with `status.lane` (derived or stored) and `status.debt`; and per team a `capacity` block — `{points}` — and a `signals` block with the five above. The watch is the same board's (`GET /api/v1/views/triage/watch`) with the same narrowing.
- `POST /api/v1/views/triage/cards` and `update_card` accept `lane` and `week`. A `lane` on a card whose lane derives is refused (422): the links say what it is.
- `POST /api/v1/teams/actions/capacity` with `team` and `points`, alongside `POST /api/v1/teams/actions/rename`; MCP `set_team_capacity`. `GET /api/v1/sprints` carries each team's `capacity.points`; the number is set, never derived.
- MCP: `list_cards view=triage` with the same selector. The board's own gestures are tools of their own: `place_card` puts the card in a week and `untriage_card` takes the week away, both doing what the REST `place`/`untriage` actions do — emptying the working area for a week AHEAD (the card is on no day board until then) and joining the team's sprint for the CURRENT one. `update_card week=<Monday>` is still the bare patch (`SetWeek`): it writes the week and nothing else, so an agent scheduling work for a later week that way has to clear the dates itself, or the card stands on today's board and in a future week at once.
...
A tool writing the repositories directly learns three things: `lane:` on a card (`client`, `plan`, `internal`; absent = needs triage; ignored on a card in an epic or with a task, where it derives), `capacity:` in `teams/<id>.yaml`, and that `week:` on a card may name a Monday ahead — such a card is on no day board until then, and the work is owed by that week's Friday. `week:` must BE a Monday: the server refuses any other day (`ErrNotAMonday`), and a file written with one stands in no column this board draws. The old `plan: wed|fri` key is gone with the weekly plan — it is read and discarded, so a writer should not emit it. Schema stays 1: older servers keep the keys they do not read. See [plugin-impact.md](plugin-impact.md).
