...
**The grid × records the day it took the card off** (P10). On a TEAM card the × demotes into the previous sprint — `start`, `day` and `sprint` all move to the previous sprint's start — and must also write **`leftAt: <today>`** on the card and its subtasks. Nothing live reads it there (today's board drops the card, which is the point of the ×), but a RECORD of that day gives the card back by it: it was worked and finished on that day, and its dates no longer say so. A plugin that demotes without the mark silently loses exactly the work that day is remembered for. Unlike the personal ×, re-dating does NOT clear it: a fact about a day gone by is not undone by moving the card again.
...
The × no longer "never deletes". A card has two homes — the working area (a sprint and its days) and the weekly plan (a band and its week) — and each × empties one of them: leaving the working area clears assignee, sprint AND dates (a slot keeps its dates), leaving the plan clears the band. A card with the other home, or a Project-board column, stays there; a card with nowhere else to be is **deleted**, whatever it carries — the UI asks first when there is progress or a linked review card to lose; subtasks are freed into standalone cards. A SUBTASK is its own case: with no column it is deleted by the grid's × (it has no sprint history of its own), and with a column it is taken OUT OF THE GROUP and left there — unless that column belongs to its PARENT's repository, which the pull-out leaves, in which case the column is dropped and the card answered like any other columnless one. The plan's × never deletes a subtask: its home is its parent. And a card is a subtask or a weekly-plan card, never both — a writer must not give a card both a `parent` and a plan band, since grouping hands the slot to the parent (the server refuses the pair at every door). A plugin replicating the old rule would hand a card back into this week's plan (the server no longer does) or refuse a delete that now happens. `model.md` must carry the two-homes rule; a plugin driving `remove_card` should ask its user before removing a worked card from its last home.
