...
| R4 | Re-review (send a passed card back to the SAME reviewer) reactivates the finished review card: original back on review (progress clamped off 100), review card progress→0, round counter ticks up (round 1 implicit, first re-review = 2). A different reviewer is a normal reassignment, not a reactivation | boardservice.ReassignReviewer + reviewRound field | ✅ TestReReviewReactivatesReviewCard / DifferentReviewer |
