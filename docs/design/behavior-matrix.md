...
| G38 | Roster names — teams, projects, processes — are one namespace across every domain: the store refuses a create or a rename into a name any domain already declares (`ErrNameTaken`, 422), readable by the caller or not; the read-side alias merge (G13) covers only collisions made behind the server's back. A team is renamed as one action — its file keeps the sprint pointer, every card and process task that names it follows; the no-team group has no name to rename | gitstore MultiBackend + boardservice.RenameTeam | gitstore `TestMultiBackendRefusesRosterNamesTakenInAnotherDomain`; boardservice `TestRenameTeamRewritesPointerAndCards`, `TestRenameTeamRefusesTakenEmptyOrNoTeam`; server `TestAPIRenameTeam`; mcpserver `TestMCPListsTools` |
