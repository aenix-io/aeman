...
| L5 | Activity log: every aeman mutation (create, stage, progress, assignee, team, zone, review sent/passed/removed, plan taken/released) records an event with the acting user, stored in the card itself (draft body log / a dedicated issue log comment) — capped at 200 events, best-effort (a log failure never fails the mutation), gone with the card on delete. GET /cards/{uid}/log + MCP list_log serve events+notes as one chronological feed; the Me day panel and CardDetail render it. Non-aeman edits (GitHub UI) leave no events; carry events are not recorded (volume) | board.Event + boardservice.logEvent + ghprojects.AppendEvent | ✅ TestEventBodyRoundTrip / TestPartitionEvents / TestMutationsRecordEvents / TestReviewCycleRecordsEvents / TestAPICardLogUnifiedFeed |
