...
| G12 | A rank insertion touches one file; an exhausted key space rebalances only the run between the nearest roomy neighbours, in the same commit, never crossing into another domain | board rank + gitstore | board `TestRankBetweenOrders`, `TestRankBetweenOpenEnds`, `TestRankBetweenRefusesEqualOrInverted`, `TestRankNeverEndsInFloorDigit`, `TestRankBetweenRepeatedInsertsStayOrdered`, `TestRankRebalanceEvenAndBounded`, `TestRankNeedsRebalance`; gitstore `TestBackendMoveCardRewritesOneFile`; 🆕 rebalance on write: gitstore `TestRebalanceStaysInDomain` (`RankRebalance` is called only by `migrate`; no write path rebalances a run yet) |
...
| G26 | Healthz reports the oldest unpushed commit age per domain and turns red past `--unpushed-warn` | server health | server `TestGitUnpushedAge`; cmd `TestGitConfigFromFlagsAndEnv` (the `degraded` flip past the threshold in `handleHealthz` has no test yet) |
