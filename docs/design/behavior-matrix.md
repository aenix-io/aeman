...
| P6 | `doneAt` is the board day a write took a card to 100, cleared when it drops below — stored like `doneFrom`, never read from history — and the resource carries it as `status.doneAt` | gitstore.SetProgress + apiserver | gitstore `TestSetProgressWritesDoneAtAndReopenClearsIt`; apiserver `TestCardResourceCarriesDoneAtAndPersonalDomain` |
