...
| G7 | A card's log = commits touching it within the horizon, across `Aeman-Moved-From`; from/to from an `Aeman-Change` trailer first, the front-matter diff second; `truncatedBefore` set when the horizon cuts it | gitstore history | gitstore `TestCardLogReadsTrailersFirstThenDiff`, `TestCardLogTruncatedBefore`, `TestMultiBackendCardLogFollowsMoveIntoOldDomain`; server `TestCardLogFromCommits` |
