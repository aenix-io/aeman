...
Companion to [git-backend.md](git-backend.md). These rules are pinned BEFORE the backend exists: each test is written first against `pkg/gitstore`, `pkg/board` or `pkg/boardservice` and must fail before the code lands. Shallow-path tests (G8–G11) run against the real `git-upload-pack`/`git-receive-pack` binaries (go-git's in-process server has no shallow support) and skip when they are absent. Rules about dates, sprints, visibility, review and carry-over do not change and keep their rows above.
...
| G3 | Derived states are never written: 100% writes `progress: 100` and `doneFrom`, never `stage: done`; In Progress is absent from every file | gitstore file format | 🆕 gitstore `TestDerivedStateNotStored` |
| G4 | One action = one commit per touched domain sharing `Aeman-Action-Id`; `Aeman-Cards` lists every id; an action that changes nothing makes no commit | gitstore commit | 🆕 gitstore `TestCarryOverIsOneCommit`, `TestActionAcrossDomainsSharesID`, `TestNoopMakesNoCommit` |
...
| G11 | Re-apply on a new tip is per-card last-write-wins; disjoint cards both land; same card → the re-applied content wins, both commits in history; two replicas spawning the same iteration → one file | gitstore sync | 🆕 gitstore `TestReapplyDisjoint`, `TestReapplySameCardLastWins`, `TestSweepIsReplicaSafe` |
...
| G14 | Domain follows the inheritance rule, never a per-card choice: project card → project's domain; team card without project → team's domain; review card → reviewed card's domain; subtask → parent's; iteration → task's | gitstore + boardservice | 🆕 `TestCardInheritsProjectDomain`, `TestTeamCardInheritsTeamDomain`, `TestReviewCardFollowsOriginal`, `TestSubtaskInheritsParentDomain`, `TestIterationFollowsTask` |
| G15 | (when `feat/card-mirrors` lands) Mirror refuses a target column in another domain (`ErrCrossDomain`); guard order exists → not own → not mirrored → same domain | boardservice.Mirror | 🆕 `TestMirrorRefusesCrossDomain` |
...
| G17 | An unreadable domain is absent from the snapshot AND the watch stream — no teams, projects, cards, placeholders or frames from it | server board composition + watch hub | 🆕 server `TestUnreadableDomainAbsent`, `TestWatchFilteredByDomain` |
...
| G22 | A cross-domain move is delete + create with the same id and one action id (`Aeman-Moved-To/From`); a torn move shows the card once, from the cache, and maintenance removes the ghost | gitstore + boardservice | 🆕 `TestMoveAcrossDomains`, `TestTornMoveServedOnce`, `TestMaintenanceRemovesGhost` |
