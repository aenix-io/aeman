...
| P5 | `view=personal` is the owner's repository as a backlog: every open card plus the ones done today; a card done on an earlier day, a card of another domain, another person's personal card — absent; the day defaults to today | board.PersonalView + apiserver | board `TestPersonalViewOpenAndDoneToday`; apiserver `TestSelectorPersonalView` |
