...
- A `parent` and a plan band together are refused (**422**), on a create and equally on a `PATCH /cards/{uid}` that would give a standing subtask a band: a subtask has no band of its own — grouping hands its slot to the parent — so the pair asks for two contradictory things. Before, the band was silently moved to the PARENT. Clearing a band on a subtask is free, which is what grouping does.
