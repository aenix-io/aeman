...
- A **Sprint**'s `spec.capacity` is what the Triage board measures a week against: `{week, client, internal}` — the team's throughput and the shares of it that client work and internal work may take. `week` is the roster's own number when a team has set one, and otherwise what the team has been FINISHING, averaged over the four complete weeks before this one; a team with no record at all reports 0, meaning the board knows no limit rather than a limit of none. Beside them, `points` is the team's week in POINTS — the capacities of the PEOPLE in it, added up, with somebody who works for more than one team split between them in proportion to what they closed in each (`pointsDerived` is always true: a team's number is derived by construction, there is nothing to set) — and `reactive` the percentage of the points the team closed that arrived outside the plan, in the unplanned and urgent zones (`reactiveKnown` is false when nothing sized was closed in the window). What a week can be planned to is `points` less that share; the Triage board draws each week's scheduled points against it.
