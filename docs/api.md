...
- **PATCH `process`** ties the card to an existing process ("" clears) — the recurring shelf's counterpart of a column. The process must live in the card's own repository, and a process TURN (a card copied from a task) is refused outright (422): a turn's process is its task's.
