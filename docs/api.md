...
- A create that names both a `parent` and a plan band is refused (**422**): a subtask has no band of its own — grouping hands its slot to the parent — so the pair asks for two contradictory things. Before, the band was silently moved to the PARENT.
