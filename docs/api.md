...
| `POST /api/v1/cards/{uid}/actions/unmirror` | `{project, epic}` | Take one mirror column away; the home and everything else stay. Both halves are required by this endpoint (422). The stored placement itself needs only the epic: a column of no project is a mirror home like any other, and the codec keeps such an entry. |
