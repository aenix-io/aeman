...
- A **Sprint**'s `spec.capacity` is what the Triage board measures a week against: `{week, client, internal}` — the team's throughput and the shares of it that client work and internal work may take. `week` is the roster's own number when a team has set one, and otherwise what the team has been FINISHING, averaged over the four complete weeks before this one; a team with no record at all reports 0, meaning the board knows no limit rather than a limit of none. Beside them, `points` is the team's week in POINTS — a different measurement, not another view of `week`, and one somebody SET: `POST /api/v1/teams/actions/capacity` with `{team, points}` writes it into the team's own file (0 takes it back; outside 0..999 is **422**), and the board derives none. The Triage board draws each week's scheduled points against the numbers of the teams on screen, added up. It was derived once — the people's capacities summed and split between their teams by what each had closed, less the share that history said arrives unplanned — and that arithmetic now lives in the `derive-capacity` skill, where how thin the record was can be said out loud beside the answer.
