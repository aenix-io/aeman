...
A person opens a BOARD and presses something on it, and which board it was decides what the press meant. That board is a **path segment**: `/api/v1/views/{view}/cards` lists what it draws, creates the kind of card it makes, and answers the gestures it offers. The views are `me`, `team`, `triage`, `backlog`, `project`, `personal`, and `all` — the escape hatch for a caller with no board at all, which has to be said out loud and is a default nowhere. `GET /api/v1/views` is the catalog: each board with the gestures it draws.
...
- **A create means what that board means by it.** Typed into `me` the card is yours, today, in the sprint, and in the **unplanned** band — that is the only one the Me board adds in, because something that came up today is unplanned by definition and the other three bands are the plan, which is the lead's to make on the team's grid (any other zone there is a 422; the personal column beside it follows the same rule); into `team` it lands in the Unassigned column unless somebody is named; into `triage` it is scheduled for a `week`, and stands on no day unless that week is the one being WORKED — a card started in the current week belongs to today as well, which is what the board's own add form in that row does, while one filed into a week ahead waits for its Monday (B1) and dates on it are a 422; into `backlog` it is parked on the team's shelf; into `project` it is a slot under a column; into `personal` it goes to your own repository. Each board refuses the fields it does not own — a parked card typed into a day, a column named on the Me board — with a **422** that says which field it was, and a board asked for the one field its whole gesture is (a Triage card with no week) answers 422 too.
- **A gesture is answered by the board that draws it.** `remove` on all of them, `place` and `untriage` on `triage`, `finished-earlier` on the day boards. Asked of another board it is **404**, the same answer as a route that is not there.
- **A gesture on a card that board does not draw is 404.** A person cannot press × on a card they cannot see; a caller that named a board is held to it. The check is the listing itself, with the same defaults (the day is today, the team the card's own, the person the caller), and it asks about the card as it stands BEFORE the write — sending work off today's board is the point of half these gestures. A board drawn from two listings counts both: the Triage grid with its drawer, the Me day with the personal column.

### Resources

| Method & path | Purpose |
| --- | --- |
| `GET /api/v1/board` | The board: title, the team roster, the Project board's structure (`metadata.projects` in board order, `metadata.epics` as `{name, project, domain}`, `metadata.deadlines` as `{week, project}`, `metadata.processes`), the people (`metadata.members`: `{login, name, avatarUrl, carrying, load, capacity}`, described below) and the visitor's domains (`metadata.domains`). |
| `GET /api/v1/views` | The boards a caller may open, each with the gestures it draws. |
| `GET /api/v1/views/{view}/cards` | LIST what that board draws (selectors below), in board order. A listing is the **board-row shape**: no `spec.description` — `status.links` carries the refs extracted from it (capped at 50), and the body itself is one `GET /cards/{uid}` away. `?fields=full` opts a genuine bulk reader into complete cards. |
| `POST /api/v1/views/{view}/cards` | Create the card that board makes (201). A title that is nothing but a GitHub issue/PR URL becomes that item's real title, with the link moved into the description (one-time, never re-synced). What each board fills in, and what it refuses, is above. |
...
Actions carry the board rules — the client never reimplements them. The four that are a BOARD's own gesture are addressed through it (`/api/v1/views/{view}/cards/{uid}/actions/…`, above); the rest mean the same thing wherever the card was opened from and are addressed as the card.

| Method & path | Body | Purpose |
| --- | --- | --- |
| `POST /api/v1/views/{view}/cards/{uid}/actions/remove` | `{intent}` | The smart ×. **`intent`** says which of its two meanings is wanted: `unassign` empties the working area and leaves the card in its week or its column, `off-board` takes it away with the subtasks that were pieces of it. Omitted, the gesture decides for itself as it always did. `off-board` on a Project-board slot or a process turn is **422** — those belong to another board's plan; `unassign` on a card with no week and no column is **422** too, since it would leave the card nowhere at all. From the **Me board** (`/views/me/...`) the × is the narrow one: a person takes off their own board what they PUT there and what the plan has not taken up — their own card, still standing in the unplanned band. Work another person scheduled for them, or work they scheduled on the Team or Triage board, is somebody's plan and is refused there (**403**); the wide × lives on the board where the planning was done, and the answer to "I am not doing this" is `spec.stage: refuse`. A subtask and a card of the caller's own personal board are outside the rule. A card the caller is CARRYING but did not create is not theirs to take off the board on any board (**403**, `DELETE /cards/{uid}` too): their answer to work somebody else planned for them is `spec.stage: refuse`. A card on somebody else is the lead's to remove, and a subtask is a piece of its parent. On a personal card: one that has been worked on (progress above 0) and did not start today is left behind on yesterday's board — `status.leftAt` set on it and its subtasks, history kept, off the personal view from today — and an untouched one, or one that started today, is deleted; re-dating a left card (`dates`, defer) brings it back. On a team card the × empties the WORKING AREA: assignee, sprint and dates cleared (a slot keeps its dates — they are its row). The card then lands wherever it still belongs — the WEEK it is scheduled for, which is its column on the Triage board, or its Project-board column. The week counts only while the card IS in the working area: one that is nothing but its week has no second home to be handed to, and an × that always answered "back to your week" could never remove such a card at all. With nowhere else to be — no week, no column — the working area was its only home, and the × deletes it, whatever the card carries. A SUBTASK standing in a column is not deleted by the × that finds it there — that column is a home of its own. The one exception is below: a column the pull-out cannot take along leaves the card columnless, and then the ordinary law applies. The × does delete a subtask with no column — there is nowhere else for it to be — and it deletes one WITH a column too when that column cannot come along: a column of the parent's repository stays behind (the card's file leaves the group for the repository its own team names), and the card is then answered like any other columnless one, which is deletion. A client should ask first when there is progress or a linked review card to lose (the UI does). This × takes the card's **subtasks with it** — they are pieces of the same work, and the demote it replaced carried them along too; a subtask standing in a column of its own is freed into that column instead. (`DELETE /cards/{uid}` still FREES a card's subtasks into standalone cards — two doors, two meanings: “this work is off the board” and “this card is a mistake” — and so does the delete cascaded onto a review card.) A deleted card is not lost to the board's history: the day it stood on gives it back (G60), which is what took the place of the demote. A card filed under a Project-board **column** (an epic) is not deleted while that column can hold it — a SUBTASK included, whose column is a home of its own even though its parent is another: the grid's × takes such a card out of its group and leaves it in the column, since a subtask's person and sprint belong to its parent and a card released while still grouped would be dragged back by the next carry-over — that column is where it goes home. A card that merely carries a project name has no column and is treated like any other. Use `DELETE /cards/{uid}` to delete deliberately. |
...
| `POST /api/v1/views/triage/cards/{uid}/actions/place` | `{week}` | Put the card in a week of the Triage board — which is what TRIAGING it means: until then nobody has said when the work is due. `week` is a **Monday**; any other day is refused (422), since a week that is not a Monday matches no column the board draws. A week AHEAD of this one also empties the working area — the card is on no day board until that Monday, and leaving a person and dates on it would say two things at once; the CURRENT week joins the team's sprint instead. A Project-board slot is refused (422): its week IS its start date's week, so its dates are what move. A process turn is held to its own occurrence here as it is on `spec.week`, with the same two exceptions. |
...
| `POST /api/v1/epics/actions/set-project` | `{epic, from, project}` | Move a column from one project to another; an empty target detaches it. Its cards are rewritten too. The column cannot change REPOSITORY, though — its stub is written back to the repository that holds it — so a target project of another one is refused (422), and the name must be free in the destination, the no-project bucket included. |
...
| `PATCH /api/v1/people/{login}` | `{capacity}` | Set the points a week a PERSON gets through — the number the Triage board measures their load against. 0 takes a set number back (nothing derives one); outside 0..999, or a login that could not be one, is **422**. Answers the whole Board resource, whose members carry `load` and `capacity`. |
...
- **Zones are semantic**: `urgent`, `unplanned`, `planned`, `niceToHave` (or empty). The UI's colours are presentation. A card in the **working area** — one carrying a sprint or dates of its own — always answers with a zone: nobody having said means `planned`, which is where every board draws it anyway. Empty is reserved for cards that are on no day (the strip, a shelf, a week to come), where "no zone yet" is a state a client draws as one. The `zone=` selector reads the same answer, so a listing and the filter never disagree about the same card.
- **`spec.size`** is what somebody said the card weighs — `S`, `M`, `L` or `XL` (any case on the way in, upper-case on the way out; anything else is **400**), or absent for an unsized card. It is set on a daily sync or by a sizing tool (`skills/size-cards`), on create or by PATCH; `""` takes it back. The points a board sums are derived from it — S 1, M 2, L 4, XL 8 — never stored, and a card with SUBTASKS weighs their sum instead of its own size (the umbrella rule: a parent's estimate stands until its children exist, then they replace it, and nothing counts twice). A card nobody has sized weighs **M** — except a **review card** (`spec.reviewOf` set), which weighs **S**: a review is somebody reading finished work and saying yes or no, and it is the one kind of card the server creates on its own, one per card sent to review, so weighing them as M put two points on a reviewer for every thing they were asked to look at. A review somebody did size keeps that size. Weighing an unsized card as nothing would tell a person their week is empty while they are drowning in it. `spec.size` stays absent on such a card; the default is what the SUMS use, not a value written to it.
...
- A `parent` and a `week` together are refused (**422**) on a create: a subtask has no week of its own — grouping hands its week to the parent, which stands for it from then on — so the pair asks for two contradictory things, and the week used to be silently moved to the PARENT. A `PATCH /cards/{uid}` that gives a STANDING subtask a week is a different request — "take it out of the group and schedule it" — and is honoured: the card is pulled out first and the week lands on it. Clearing a week is free, which is what grouping does.
- A **personal** create (`POST /views/personal/cards`) with a `parent` takes only a parent of your own board: a personal card's file lives in your repository, so a parent elsewhere would put the pair in two (422).
...
- **`spec.stage`** is `locked`, `review`, `recurrent`, `refuse` or empty — and **only** those: a name the board does not have is refused (400 at the body parse, 422 through the service and MCP) rather than stored, since a card wearing a stage nothing knows reads as ordinary work in progress. Done is **derived** (`status.complete`): 100% with no stage. **`refuse`** is the answer of the person the card is on — "I am not doing this" — so only they may set it (**403** otherwise); it does not take the card off anyone's board, it leaves it standing for their lead to answer. Clearing it IS that answer, and anyone may. Patching `stage: "done"` clears the stage and fills 100%; review/locked clamp progress to [10, 90]. A REVIEW card itself takes neither `review` nor `recurrent` (**422**): a review of a review is a chain nothing draws, and the asking is one-off by nature. Taking a card off review cancels its unfinished linked review card server-side: the card is DELETED, and the cancellation is recorded on the original, where a person looks for what became of the review. A review the reviewer has already worked on (progress above 0) is left alone — that work is theirs — and a finished review card stays as the record that the review happened. Reopening a done card restores the progress it had when it was marked done.
- **`status`** is server-derived and read-only: `complete`, `inProgress`, `overdue` (a broken promise made on ANOTHER board — a Project-board slot past its end date, a process turn past the week its process filed it into. A card the Triage board scheduled into a week is not marked late for still being open: that week is this board's own planning, which the next sync redoes. It is still a DEBT: it stays in the week it was owed in, and the day boards go on drawing the work until it is finished, W3), `reviewedBy` (the assignee of the unfinished linked review card), `domain` (the repository the card lives in), `doneAt` (the board day the card reached 100, cleared on reopen), and `links` — the references extracted from the description (unresolved; `GET /cards/{uid}/links` resolves GitHub refs to live titles and states).
- A **Sprint**'s `spec.capacity` is what a week of the team's plan is weighed against: `{points}` — the points a week the team gets through, a number somebody SET and never derived, absent when nobody has said. `POST /api/v1/teams/actions/capacity` with `{team, points}` writes it into the team's own file (0 takes it back; outside 0..999 is **422**), and the Triage board draws each week's scheduled points against the numbers of the teams on screen, added up. It carried four more fields until v0.37 — `week` (a limit in CARDS a week, derived from what the team had been finishing), `client`, `internal` and `derived` — which nothing wrote and no client read; the arithmetic that filled them in, and the sum-of-people split that briefly filled in `points`, now live in the `derive-capacity` skill, where how thin the record was can be said out loud beside the answer.
- **`spec.parked`** puts the card on its team's SHELF rather than in the plan. A shelf is a THIRD place a card can be, beside a week and the strip: work somebody has read and put aside. It is exclusive with `spec.week` — setting one clears the other, at every door that sets one — and a parked card is on no day board and out of the strip. Parking also empties the working area (person, sprint, dates), takes the card's subtasks off the board with it, withdraws any unfinished review of it exactly as leaving the review stage does, and puts it at the TOP of the shelf. A Project-board slot or a process turn is refused (**422**) — their week is another board's to say — and so are a REVIEW card and a SUBTASK: neither has a place of its own to be parked out of, one follows the card it reviews and the other stands inside its parent, so a shelf would take it out of the only place it is drawn.

  Every team **has** a backlog and nothing declares it — the no-team group included, which is a team like any other here. It cannot be created, renamed or removed, so no card is ever left pointing at a shelf that has gone and parking can never be refused for want of one. Parking never changes a card's `team`; to move parked work to another team, send `team` and `parked` in the same patch — the team is applied first. `POST /cards` takes `parked` too: the card is born on the shelf, with no dates and no sprint.
...
```json
{
  ...
  "metadata": {
    ...
    "projects": ["cozystack"],
    "epics": [{ "name": "Auth", "project": "cozystack", "domain": "closed" }],
...
Each member carries, beside `carrying` (open cards on the person, counted across every team), `load` — the same cards weighed in points — and `capacity`, the points a week the person gets through: the roster's number, and only that. It is absent when nobody has set one; the board does not read a capacity off its own record, because `doneAt` only goes back to the day the board started keeping it and a median over a half-empty window reads as a fact while being an artefact of the record. The `derive-capacity` skill is that arithmetic, for a lead to run and write the answer back. A board draws `load/capacity` over the person and goes red past it, and with no capacity draws the load alone. `PATCH /api/v1/people/{login}` with `{"capacity": 40}` sets it (0 takes a set number back; outside 0..999 is **422**) and answers with the whole Board resource.
...
`teamDomains`, `projectDomains` and `processDomains` name the repository a team, a project or a process was declared in. The git server names EVERY entry it declares, the primary's included and whatever the board spans: a domain always has a name (`--repo name=url`, derived from the URL when the name is left out), and the reader stamps every entry with the name of the repository it was read from. So read these as NAMES and compare them against `domains[0].name` — never for emptiness. An entry missing from a map means the primary too (a hand-built board, an older server), which is why the comparison is `entry ?? domains[0].name`; treating an absent stamp as "one repository" and a present one as "another" is the mistake these maps exist to prevent. They exist because **a card's team and its project must live in the same repository**: the project decides where the card lives, so a team from another repository would leave the card where its own people cannot read it. A write that would pair them — `PATCH /cards/{uid}` with a team or a project, a create, or an epic moved between projects — is refused with **422** naming both repositories, so a client narrows its own pickers with these two maps rather than offering a choice the server will reject. A name the roster does not declare yet is not a conflict: it is declared in the card's own repository on the way.

A card's **column** obeys the same closure from the other side, and a client narrowing a column picker cannot do it from `projectDomains` alone. The column must live in the repository that holds the card's FILE — which is usually the card's project, but not when a link outranks it (G14): a subtask's file rides its parent, a review card's follows its original. For those, compare the card's own `status.domain` against the target column's repository — which every column now carries: `metadata.epics[].domain` — the repository the column was declared in, which a multi-repository server names for every column, the primary included, and a hand-built board may leave empty for the primary; compare it as a name, not for emptiness. It is the COLUMN's, never its project's: one project name may be declared in two repositories with its columns merged under a single entry, and then the two differ. Every act that could break the pair is refused with **422**: attaching or moving the card to such a column, grouping it under a parent elsewhere, pulling it back out, re-teaming it, or setting `reviewOf` — and equally moving the card whose file others FOLLOW, while any of them stands in a column of the repository being left.
...
- `/views/me/cards?user=octocat&day=` — the personal day view for a specific user (empty user = the caller).
