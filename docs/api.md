...
Each member carries, beside `carrying` (open cards on the person, counted across every team), `load` — the same cards weighed in points — and `capacity`, the points a week the person gets through: the roster's number when a lead set one, otherwise derived from the points the person closed in each of the last four complete weeks in which they closed anything (`capacityDerived`; this week never counts, weeks with nothing closed are skipped, and no record at all is 0 and derived — "no history", not "no room"). The day boards draw `load/capacity` over the person and go red past it. `PATCH /api/v1/people/{login}` with `{"capacity": 40}` sets it (0 takes a set number back; outside 0..999 is **422**) and answers with the whole Board resource.
