...
- **`spec.size`** is what somebody said the card weighs — `S`, `M`, `L` or `XL` (any case on the way in, upper-case on the way out; anything else is **400**), or absent for an unsized card. It is set on a daily sync or by a sizing tool (`skills/size-cards`), on create or by PATCH; `""` takes it back. The points a board sums are derived from it — S 1, M 2, L 4, XL 8 — never stored, and a card with SUBTASKS somebody has sized weighs their sum instead of its own size (the umbrella rule: a parent's estimate stands until its children exist, then their sum replaces it, and nothing counts twice).
