...
A slot's **row is the week of its `dates.start`** — always derived, never stored beside the dates where the two could drift. `spec.week` is the week a card with no dates is scheduled for, which is its column on the Triage board; setting it on a card filed under an epic is refused (422).
...
| `delete_card` / `remove_card` | Hard delete; the smart ×. |
