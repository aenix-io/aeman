...
| `POST /api/v1/sprints/actions/carry-week` | `{team, week, dryRun}` | File the process turns the week is owed and reseed finished recurrent cards. Unfinished plan cards are **not** moved — a debt stays in the week it was owed in and shows on the current week's panel as overdue; `carried` counts them. The server runs this by itself as each week arrives. |
...
| `carry_over` / `carry_week` | `carry_over` advances a team's sprint. `carry_week` forces the weekly sweep the server already runs: process turns filed (`spawned`), finished recurrent cards reseeded, debts counted (`carried`) — never moved. |
