...
| `POST /api/v1/cards` | Create a card (201). A title that is nothing but a GitHub issue/PR URL becomes that item's real title, with the link moved into the description (one-time, never re-synced). `personal: true` files the card on the caller's personal board instead — with a team, column or week it is a 422. Work you file for YOURSELF is unplanned work: a create that assigns the card to the caller in any other zone is a **403** — the planned zones are the plan, made with the team. Planning somebody else's week is untouched, as is a card placed by the thing it belongs to (a column, a parent, a review). |
...
- A **Sprint**'s `spec.capacity` is what the Triage board measures a week against: `{week, client, internal}` — the team's throughput and the shares of it that client work and internal work may take. `week` is the roster's own number when a team has set one, and otherwise what the team has been FINISHING, averaged over the four complete weeks before this one; a team with no record at all reports 0, meaning the board knows no limit rather than a limit of none.
- **`status.triage`** marks a card the Triage board's **strip** holds: an open card of its own — no parent, no original — that nobody has placed in a week. **`status.triageWeek`** is the Monday of the column it stands in, which is its week; absent means it stands in none. A card sent to review is in neither: its work is done and the question is somebody else's.
- **`status.due`** and **`status.cycle`** are the weeks work still OWES, sent so a client need not reckon a calendar of its own — a second answer to "when is this next due" is a second answer waiting to disagree. `due` is a **recurrent** card's coming rounds: the Mondays, over the planning horizon, in which the card comes round again and no copy of it stands yet (absent on everything else, and on a recurrence with no calendar). `cycle` is a process **turn's** own occurrence, `{from, to}` as Mondays both inclusive: the first and last week that turn may stand in. A turn carried past the next due date would stand where the NEXT turn belongs, and the two would then read as one process running twice — so a client bounds the drag by this rather than by guessing. Absent on anything that is not a turn, and on a turn whose task has no calendar. `status.links` is what lets a listing drop the description without blinding a row's links indicator.
...
Three things to know on macOS. The item is written through `/usr/bin/security`, which puts it in the stable `apple-tool` access partition: that is what lets a rebuilt aeman still read what it stored, and the trade is that any process running as the same user can read it without a prompt. That same route passes the token to `security add-generic-password -w` as a command-line argument (base64-wrapped, so that arbitrary bytes survive argv, not to hide anything), which means the secret appears in the exec event — an endpoint-security agent that records full argv records the token along with it. The read audience is no wider than the paragraph above already says; the recorded audience is. And the item lives in the login keychain, which is unlocked by logging in, so a service that reads it must run as a LaunchAgent in the user's session rather than as a LaunchDaemon.
