...
- `internal/tokenstore` — the forge credential in the OS secret store, one token per forge host, written by `aeman login`; it is also a `forge.CLI`, so the keychain is a source like the forge's own tools.
- `internal/server` — HTTP/WS, OAuth sessions (self-hosted mode) vs the local single-user identity, which is whoever the credential chain's elected source holds a token for (the forge's variables, `internal/tokenstore`, then `internal/ghcli` or `internal/glabcli`; the chain itself is `cmd/aeman/forge.go`), the board cache and write queue, the git sync (`gitsync.go`, `gitmode.go`), per-visitor domain rights from the forge (`access.go`, `visible.go`).
...
- `web/` — the React SPA. The domain rules are **mirrored** in the frontend, and the mirrors are files, not scattered component code: `web/src/date.ts` and `web/src/sprint.ts` (the date and sprint model), `web/src/slots.ts` (what makes a card a Project-board slot, and the week that follows its dates), `web/src/removal.ts` (what each × does, and the optimistic patch it leaves), `web/src/size.ts` + `web/src/load.ts` (what a card weighs, the umbrella rule, and how a person's or a week's load stands against a capacity), `web/src/placements.ts` + `web/src/domains.ts` (columns, mirrors and the one repository comparison), `web/src/viewquery.ts` (what each view asks the server for, including which days are shown as a snapshot — the same condition the server applies), `web/src/teamgrid.ts` and `web/src/meview.ts` (what the Team and Me boards draw on a day, card by card), with `MeBoard.tsx`/`TeamBoard.tsx`/`ProjectBoard.tsx` calling them rather than re-deciding. A change to a filter, date, sprint, band, removal or domain rule lands in both the Go and the TS copy — with a vitest case beside the Go one — or the optimistic UI diverges from the server. The /api/v1 wire shapes are not mirrored by hand at all: `web/src/api/schema.d.ts` is generated from `api/openapi.yaml`, `web/src/api/resources.ts` names those shapes and maps them onto the internal Card model, and `web/src/providers/api/apiProvider.ts` calls the same document through `openapi-fetch` — so a field the document renames or drops is a compile error in the mappers rather than a runtime surprise.
