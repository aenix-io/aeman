// Package ghsource is the read-only GitHub Projects v2 client behind
// `aeman migrate`: it loads one project board over the GraphQL API and maps
// it onto the domain board.Board the migration consumes. It deliberately
// carries no write path — the git layout is the only store aeman writes to.
package ghsource
...
// Sentinel errors returned by the client; callers tell them apart with
// errors.Is.
var (
...
// Client reads GitHub Projects v2 boards over the GraphQL API with a fixed
// bearer token.
type Client struct {
	token      string
	endpoint   string
